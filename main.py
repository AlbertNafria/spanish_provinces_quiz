...
provinces_missing_csv_file = "provinces_missing.csv"
...
while len(guessed_provinces) < len(provinces_list):
    ...
    if answer == "Exit":
        missing_provinces = [province for province in provinces_list if province not in guessed_provinces]
        # Save a list of missing provinces
        missing_provinces_data = pandas.DataFrame(missing_provinces)
        missing_provinces_data.to_csv(provinces_missing_csv_file)
        break
